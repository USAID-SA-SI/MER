...
options(gargle_oauth_email = "ctrapence@usaid.gov")
options(gargle_oauth_email = TRUE)
rive_auth(email = TRUE)
...
#df_fac <- clean_mfl()
#df_fac2<-df_fac %>%  rename(organisationUnit=usaid_facility,orgUnitUID=datim_uid  ) %>% select(organisationUnit,orgUnitUID)
...
FY22_pepfar<-list.files(here("Data"),pattern="_FY22Q4")

FY22_pepfarv1<-rbind(df_fac2,FY22_pepfar) %>% mutate(organisationUnit=str_replace(organisationUnit,'ec Tafalofefe Gateway Clinic','ec Tafalofefe Hospital'))

FY22_pepfarv1<-rbind(df_fac2,FY22_pepfar) %>% mutate(organisationUnit=str_replace(organisationUnit,'ec Rodana Clinic','ec Rode Clinic'))



#FY22_pepfar<-read.xlsx(here("Data",FY22_pepfar)) %>% select(organisationUnit,orgUnitUID) %>% distinct(orgUnitUID,.keep_all = TRUE)
  
  
  #FY22_pepfar<-sqldf('select * from  FY22_pepfar where mechCode in (70291,80064,80065,81891,81892,83012,83013,160584,160585,160588,160589,160590,160591)')
                                                                                                                    
                                                                                                                
hierarchy<-Genie %>% 
  filter(fiscal_year=="2023") %>% 
  distinct(mech_code,mech_name,orgunituid,indicatortype,sitename,operatingunit,
...
tempfile<-read.xlsx((here("Data",NDOH)),sheet = "TB_ART (2)" ) 

#tempfile<-inner_join(tempfile,FY23_pepfar,by=c("new_ou5_code")) 
tempfile<-left_join(tempfile,FY22_pepfarv1,by=c(" orgUnitUID"="orgUnitUID"),multiple = "all") %>% select(Province,District,SubDistrict ,Facility ,Code  ,FineAgeGroup ,Sex  ,ARTStatus ,Total,orgUnitUID  )

tempfile<-left_join(tempfile,hierarchy,by=c("orgUnitUID"="orgunituid"),multiple = "all") %>% select(Province,District,SubDistrict ,Facility ,Code  ,FineAgeGroup ,Sex  ,ARTStatus ,Total ,indicatortype) %>% rename(dsd_ta=indicatortype) %>% mutate(FineAgeGroup=case_when(FineAgeGroup=="50-54" |FineAgeGroup=="55-59"|FineAgeGroup=="60-64" | FineAgeGroup=="65+"~"50+",TRUE ~FineAgeGroup)) %>% 
  mutate(dataelementuid=if_else(dsd_ta=="TA","Szuf9YjHjTL","Qc1AaYpKsjs"))%>% 
  mutate(category_temp=case_when(Sex=="Female" & (FineAgeGroup=="<1" & ARTStatus=="New on ART") ~"cW1wQgs5hyV",Sex=="Male" & (FineAgeGroup=="<1" & ARTStatus=="New on ART") ~"MQZqURahCb8",  
                                 ...
                                 unite(key,dataelementuid,category_temp,sep="_") %>% rename(facility=Facility,value=Total)
                                  

# MERGE IMPORT REF TABLES FROM GENIE & DATIM API
final_df<-tempfile %>% 
  left_join(data_elements,by="key")  %>% 
  left_join(hierarchy,by="facility",multiple = "all")%>% 
  mutate(mech_code=as.character(mech_code))  %>% select(-ARTStatus,-key,-Sex,-dsd_ta  ,-FineAgeGroup,-facility,-SubDistrict,-District, -Province)%>% 
   rename_official()  %>% 
